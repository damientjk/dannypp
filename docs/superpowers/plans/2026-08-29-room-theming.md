...
- **Decor layering**: the spec's §4 described per-item layering (a wall-mounted piece like a bookshelf drawing under a character, a floor piece like a yoga mat sitting on the old `furniture-below` tile layer). Task 4's implementation puts every decor item in one shared container below every character, with no per-item distinction — simpler, and correct for every item actually placed across all 6 rooms (nothing needed to draw *above* a character). Not called out at the time Task 4 shipped; recorded here during the final review that caught the gap between spec text and what got built.
